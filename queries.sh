...
or team_id in 
(select opponent_id from games where round = 'Eighth-Final' and year = 2014) order by name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "select name from teams 
where team_id in (select distinct(winner_id) from games) order by name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "select year, name from games left join teams on 
